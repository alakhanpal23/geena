...
const SCORE_KEYS = ["diction", "emotion", "intent", "dynamics", "presence", "bollywoodTone"];
...
const DEFAULT_GRADE = {
  scores: { diction: 5, emotion: 5, intent: 5, dynamics: 5, presence: 5, bollywoodTone: 5 },
  overall: 5,
  castingNote: "Focus on clarity and emotional truth.",
  notes: ["Work on variety.", "Commit to the beat change.", "Keep consonants crisp."],
  drill: "Repeat key lines with clearer articulation.",
  take2: "Take 2: Try again with more energy and a clearer beat change.",
};

/** Normalize model output so it passes validateGradeOutput (snake_case, string numbers, etc.). */
export function normalizeGradeOutput(obj) {
  if (!isPlainObject(obj)) return { ...DEFAULT_GRADE };
  const get = (o, ...keys) => {
    for (const k of keys) {
      const v = o?.[k];
  ...
  const scoresIn = isPlainObject(obj.scores) ? obj.scores : {};
  const scoresOut = {};
  for (const k of SCORE_KEYS) {
    scoresOut[k] = clampScore(scoresIn[k] ?? scoresIn[k.replace(/([A-Z])/g, "_$1").toLowerCase()]);
  }
  const overall = clampScore(get(obj, "overall"));
  const castingNote =
    String(get(obj, "castingNote", "casting_note", "casting note") ?? "").trim() ||
    "Focus on clarity and emotional truth.";
  ...
  const notesOut = notes
    ...
    .map((n) => String(n ?? "").trim())
    .filter(Boolean);
  while (notesOut.length < 3) notesOut.push("Keep working on variety and intention.");
  const drill =
    String(get(obj, "drill") ?? "").trim() ||
    "Repeat key lines with clearer articulation and pacing.";
  const take2 =
    String(get(obj, "take2", "take_2", "take 2") ?? "").trim() ||
    "Take 2: Try again with more energy and a clearer beat change.";
  return {
    scores: scoresOut,
    overall,
    castingNote,
